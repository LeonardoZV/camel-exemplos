package br.com.leonardozv.captura.eventos.controllers;
...
import br.com.leonardozv.captura.eventos.services.CacheParticoesComFalhaService;
import br.com.leonardozv.captura.eventos.services.TopicoConsumoEventoNegocioService;
...
public class ConsumoEventoNegocioKafkaController {
	...
	public boolean causaExceptionEhInterruptException(Exchange exchange) {
	
		Exception exception = exchange.getProperty(Exchange.EXCEPTION_CAUGHT, Exception.class);
		
		if (exception instanceof org.apache.kafka.common.errors.InterruptException) {
			return true;
		}
	...
	public String formatarLogErro(Exchange exchange) {
		...
		String offset = (String)exchange.getIn().getHeader("kafka.OFFSET").toString();
		...
		return "Topico: " + topico + " - Partição: " + particao + " - Offset: " + offset + " - Exception: " + sw.toString();
